...
from python_scripts.PPO.cbam_modules import CBAM, SequenceCBAM
import csv
import os
class ReplayBuffer:
...
class ActorCritic(nn.Module):
    def __init__(self, act_dim):
        ...
        # CBAM-based fusion blocks for image/state and temporal features
        self.image_feature_cbam = SequenceCBAM(128)
        self.graph_feature_cbam = SequenceCBAM(128)
        self.temporal_image_cbam = SequenceCBAM(128)
        self.temporal_state_cbam = SequenceCBAM(128)
        self.spatial_fusion_proj = nn.Linear(256, 256)
        self.spatial_fusion_cbam = SequenceCBAM(256)
        self.temporal_fusion_proj = nn.Linear(256, 256)
        self.temporal_fusion_cbam = SequenceCBAM(256)
        self.final_fusion_proj = nn.Linear(512, 512)
        self.final_fusion_cbam = SequenceCBAM(512)
    ...
    def _normalize_inputs(self, img, state):
        ...
        """
        # state (先规范化 state，用其 batch 维度辅助图像三维输入消歧)
    ...
    def _encode_temporal_sequence(self, current_feature, buffer_deque, cbam_module):
        """
        current_feature: [B, 1, 128]
        For single-sample online inference, use buffered sequence and return the latest token.
        For batched training, enhance the current token directly.
        """
        batch_size = current_feature.shape[0]
        if batch_size == 1:
            buffer_deque.append(current_feature.detach().cpu())
            seq = self._stack_buffer(buffer_deque)
            if seq is not None:
                seq = seq.squeeze(1).unsqueeze(0)  # [1, T, 128]
                seq = cbam_module(seq)
                return seq[:, -1:, :]
        return cbam_module(current_feature)
    ...
    def forward(self, img, state):
        ...
        initial_image_features = self.image_feature_cbam(p_fc.unsqueeze(1))  # [B,1,128]
        ...
        initial_state_features = self.graph_feature_cbam(initial_state_features)
        ...
        # --------------------------
        vision_graph_out = initial_image_features.repeat(1, initial_state_features.shape[1], 1)
        spatial_features = torch.cat([vision_graph_out, initial_state_features], dim=-1)
        spatial_features = self.relu(self.spatial_fusion_proj(spatial_features))
        spatial_features_out = self.spatial_fusion_cbam(spatial_features)

        # --------------------------
        # Temporal branch (USE raw features for BOTH image & state)
        # --------------------------
        temporal_img_out = self._encode_temporal_sequence(
            initial_image_raw_features, self.buffer_img, self.temporal_image_cbam
        )
        temporal_state_out = self._encode_temporal_sequence(
            state_raw, self.buffer_state, self.temporal_state_cbam
        )
        temporal_features = torch.cat([temporal_img_out, temporal_state_out], dim=-1)  # [B,1,256]
        temporal_features = self.relu(self.temporal_fusion_proj(temporal_features))
        temporal_features_out = self.temporal_fusion_cbam(temporal_features)

        # --------------------------
        # Final fusion of spatial and temporal features (unchanged)
        # --------------------------
        spatial_summary = spatial_features_out.mean(dim=1, keepdim=True)
        temporal_summary = temporal_features_out.mean(dim=1, keepdim=True)
        final_features = torch.cat([spatial_summary, temporal_summary], dim=-1)  # [B,1,512]
        final_features = self.relu(self.final_fusion_proj(final_features))
        final_features_out = self.final_fusion_cbam(final_features)
