...
from Spatiotemporal_attention_mechanism.temporal_attention.causal_attention import CausalSelfAttention, CausalCrossAttention
...
class ActorCritic(nn.Module):
    def __init__(self, act_dim):
        ...
        # --------------------------
        # CausalSelfAttention implementations assumed present in file
        self.tem_attn1 = CausalSelfAttention(128, 4, 0.1)
        self.tem_attn2 = CausalSelfAttention(128, 4, 0.1)

        # Causal cross attention fusion layers
        self.spatial_vision_graph_attn = CausalCrossAttention(dim=128, heads=4, dim_head=32, prenorm=True)
        self.spatial_fusion_attn = CausalCrossAttention(dim=256, heads=8, dim_head=32, prenorm=True)
        self.temporal_vision_graph_attn = CausalCrossAttention(dim=128, heads=4, dim_head=32, prenorm=True)
        self.temporal_fusion_attn = CausalCrossAttention(dim=256, heads=8, dim_head=32, prenorm=True)
        self.final_fusion_attn_front = CausalCrossAttention(dim=256, heads=8, dim_head=32, prenorm=True)
        self.final_fusion_attn = CausalCrossAttention(dim=512, heads=16, dim_head=32, prenorm=True)
    ...
    def forward(self, img, state):
        ...
        if seq_imgs is not None and seq_imgs.shape[0] > 0:
            # CausalSelfAttention: 内部自带因果mask，只看到≤当前时间步
            tmp_img_attn = self.tem_attn1(seq_imgs.permute(0, 1, 2))
            ...
            temporal_img_out = initial_image_raw_features
